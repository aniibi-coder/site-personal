...
const WINDOWS = {
  ...
  folder_experience: {
    ...
    html: () => explorerView({
      ...
      items: [
        { type: "file", icon: "📄", name: "Work_Experience.txt", meta: "TXT", open: "experience_txt" },
        { type: "file", icon: "📄", name: "Education.txt", meta: "TXT", open: "education_txt" },
        { type: "file", icon: "📄", name: "Skills.txt", meta: "TXT", open: "skills_txt" },
      ],
    }),
  },
  ...
  folder_projects: {
    ...
    html: () => explorerView({
      ...
      items: [
        { type: "file", icon: "📄", name: "Ridekit_fr.txt", meta: "TXT", open: "proj_ridekit" },
        { type: "file", icon: "📄", name: "Mental_AI_Assistant.txt", meta: "TXT", open: "proj_ai" },
        { type: "file", icon: "📄", name: "Clothing_Brand.txt", meta: "TXT", open: "proj_clothing" },
      ],
    }),
  },
  ...
  folder_contact: {
    ...
    html: () => explorerView({
      ...
      items: [
        { type: "link", icon: "✉️", name: PROFILE.email, meta: "Email", href: `mailto:${PROFILE.email}` },
        { type: "link", icon: "📞", name: PROFILE.phone, meta: "Phone", href: `tel:${PROFILE.phone.replace(/\s+/g, "")}` },
        { type: "link", icon: "🔗", name: "LinkedIn", meta: "URL", href: PROFILE.linkedin },
      ],
    }),
  },
...
function explorerView({path, items}){
  const iconKeyFor = (it) => {
    if (it.icon) return it.icon;                    // explicit override
    if (it.type === "folder") return "folder";
    if (it.type === "computer") return "computer";
    if (it.type === "bin") return "bin";
      ...
      ${items.map((it, idx) => {
        ...
        return `
          <div class="file" data-idx="${idx}" data-type="${escapeAttr(it.type)}"
            ...
            <div class="file__icon" aria-hidden="true"><span class="ico ico--${escapeAttr(ico)}"></span></div>
